...
struct mydsp : public dsp {
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("freq", &fHslider0, FAUSTFLOAT(0.0f), FAUSTFLOAT(-24.0f), FAUSTFLOAT(48.0f), FAUSTFLOAT(0.1f));
		ui_interface->addHorizontalSlider("thresh", &fHslider1, FAUSTFLOAT(-5e+01f), FAUSTFLOAT(-1e+02f), FAUSTFLOAT(1e+01f), FAUSTFLOAT(0.1f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = std::pow(1e+01f, 0.05f * float(fHslider1));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = std::fmod(fRec0[1] + 1001.0f - fSlow0, 1e+03f);
			float fTemp0 = std::min<float>(0.0078125f * fRec0[0], 1.0f);
			float fTemp1 = fRec0[0] + 1e+03f;
			float fTemp2 = std::floor(fTemp1);
			float fTemp3 = float(input0[i0]);
			fRec2[0] = fConst2 * std::fabs(fTemp3) + fConst1 * fRec2[1];
			int iTemp4 = fRec2[0] > fSlow1;
			iVec0[0] = iTemp4;
			iRec3[0] = std::max<int>(iConst3 * (iTemp4 < iVec0[1]), iRec3[1] + -1);
			float fTemp5 = std::fabs(std::max<float>(float(iTemp4), float(iRec3[0] > 0)));
			float fTemp6 = ((fTemp5 > fRec1[1]) ? fConst1 : fConst4);
			fRec1[0] = fTemp5 * (1.0f - fTemp6) + fRec1[1] * fTemp6;
			float fTemp7 = fTemp3 * fRec1[0];
			fVec1[IOTA0 & 2047] = fTemp7;
			int iTemp8 = int(fTemp1);
			int iTemp9 = int(fRec0[0]);
			float fTemp10 = std::floor(fRec0[0]);
			output0[i0] = FAUSTFLOAT((fVec1[(IOTA0 - std::min<int>(1025, std::max<int>(0, iTemp9))) & 2047] * (fTemp10 + (1.0f - fRec0[0])) + (fRec0[0] - fTemp10) * fVec1[(IOTA0 - std::min<int>(1025, std::max<int>(0, iTemp9 + 1))) & 2047]) * fTemp0 + (fVec1[(IOTA0 - std::min<int>(1025, std::max<int>(0, iTemp8))) & 2047] * (fTemp2 + (-999.0f - fRec0[0])) + fVec1[(IOTA0 - std::min<int>(1025, std::max<int>(0, iTemp8 + 1))) & 2047] * (fRec0[0] + (1e+03f - fTemp2))) * (1.0f - fTemp0));
			fRec0[1] = fRec0[0];
			fRec2[1] = fRec2[0];
			iVec0[1] = iVec0[0];
			iRec3[1] = iRec3[0];
	...
	FAUST_ADDHORIZONTALSLIDER("freq", fHslider0, 0.0f, -24.0f, 48.0f, 0.1f);
	FAUST_ADDHORIZONTALSLIDER("thresh", fHslider1, -5e+01f, -1e+02f, 1e+01f, 0.1f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, freq, "freq", fHslider0, 0.0f, -24.0f, 48.0f, 0.1f) \
		p(HORIZONTALSLIDER, thresh, "thresh", fHslider1, -5e+01f, -1e+02f, 1e+01f, 0.1f) \
